...
if uploaded_file is not None:
    ...
    st.write("Raw Data:")
    st.write(data.iloc[1:].to_string(index=False, header=False))  # Remove the first data row
    ...
    try:
        time_before_calibration = data[data[1] >= s_calibration][0].iloc[1] - 1.0  # Adjusted to skip first row
    except IndexError:
    ...
    if time_before_calibration:
        ...
        st.write(trimmed_data.iloc[1:].to_string(index=False, header=False))  # Remove the first data row
